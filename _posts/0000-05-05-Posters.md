...
\end{minipage}
```
You can use commands like <code>\bf</code> or <code>\textbf{...}</code> to print (parts of) the header in bold type. You can change the size of the font by playing with the commands like <code>Large</code> and <code>VeryHuge</code>. See this [Reference Guide](https://www.overleaf.com/learn/latex/Font_sizes,_families,_and_styles#Reference_guide) for a list of available size options.
...
\usecolortheme{...}
```
You can choose which theme and colour you would like from the very handy [Beamer Theme Matrix](https://hartwork.org/beamer-theme-matrix/). You can then fill in the names of the theme an colour you choose on the dots. You can also use the UvA beamer theme, which can be downloaded by pressing the download icon in the right corner of the grey rectangular title [here](https://github.com/UvA-FNWI/LaTeX/blob/gh-pages/week3/uvamath/BeamerthemeUvA-FNWI.zip). 
