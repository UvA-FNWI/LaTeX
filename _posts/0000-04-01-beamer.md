...
\end{frame}
```
The enumeration in the angle brackets in the `\uncover` command indicates the order at which these will appear on the slide, i.e. `\uncover<1->` will appear first and `\uncover<10->` will appear as tenth. This enumeration holds within a specific frame, in the next frame it resets and you can start again from `\uncover<1->`. Note that `\uncover<1-3>` means the specific content will appear only on the first three slides, after that it will disappear again.  
...
You only get a short time. Keep this in mind when
