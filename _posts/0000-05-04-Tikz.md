...
```latex
\begin{tikzpicture}[scale=3]
  \draw (0,0) node[circle]{$\sum_{i=1}^{n}n^2$} -- (1,1) node[rectangle,draw]{$\frac{1}{\sqrt{2}}$};
\end{tikzpicture}
```
...
% Definieer een gelijkzijdige driehoek met linkeronderhoek op coördinaat #1 en met lengte van de zijdes #2
  ...
  \tikzmath{
    ...
    function sierpinski(\x, \y, \s, \d) {
      if (\d == 0) then {
        ...
        % Rescale de lengte van de zijdes en kies de juiste coordinaten voor de nieuwe driehoekjes
        \u1 = 0.25*\s;
        \u2 = \u1*sqrt(3);
        \u3 = 0.5*\s;
    ...
    \S = 4;
    for \d in {0,...,5}{
      % Om alle plots netjes langs en onder elkaar te krijgen definieren we de coordinaten van linkeronderhoek vooraf
      \x = (\S+1)*mod(\d,2);
      \y = int(\d/2) * (\S+1);
      sierpinski(\x,-\y,\S,\d);
      };
