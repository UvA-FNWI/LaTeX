...
5.  Compare to the [source](https://nl.wikipedia.org/wiki/%C3%89variste_Galois) on Wikipedia. Correct the spacing. Check if all accents are typeset correctly (this will depend on your version of TeXworks). Correct any missing accents if necessary. Find out how (use Wikibooks/LaTeX).
6.  Find the paragraph break `\par` which is not done by a blank line. Change the code to a blank line.
7.  In LaTeX it is standard to typeset a new paragraph on the next line, with an indent before the text. Some people like to have paragraphs separated by empty lines instead. Try putting `\usepackage{parskip}` before `\begin{document}` but after `\documentclass[a4paper]{article}`, and see what this changes, in the document.
...
- Format your code the same ways the text is supposed to be typeset, i.e. put the `\[`and `\]`-commands on a separate line in the code, with the formula to be displayed in between them.
- Try to avoid excessive "hacking" to make the document look the way you want. So use `\usepackage{parskip}` if you like the empty lines better in stead of forcing them using `\, \\` or something equally distracting looking.
</div> </div>
